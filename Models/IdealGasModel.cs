...
namespace ParticleInteractionModel.Models
{
    public class IdealGasModel : PhysicalModel
    {
        ...
        public override void CollisionHandler(Ball obj_1, Rect obj_2)
        {
            ...
            double delta_x = obj_1.Position.X > (RightBorder - LeftBorder) / 2 + LeftBorder ?
                                (obj_1.Position.X + obj_1.Radius) - RightBorder :
                                 LeftBorder - (obj_1.Position.X - obj_1.Radius);


            // Вычисление на сколько край шара сместился за границы сосуда по оси Y (delta_y)
            double delta_y = obj_1.Position.Y > (DownBorder - UpBorder) / 2 + UpBorder ?
                            (obj_1.Position.Y + obj_1.Radius) - DownBorder :
                            UpBorder - (obj_1.Position.Y - obj_1.Radius);

            // Вычисление коэффицента смещения, который показывает за сколько итераций шар пройдёт delta с текущей скоростью
            double k_y = Math.Abs(delta_y / obj_1.Velocity.Y);
            double k_x = Math.Abs(delta_x / obj_1.Velocity.X);

            // Вычисление скорости по оси X при соудорении с левой или правой стенкой и смещение шара назад на delta_x
            if (obj_1.Position.X + obj_1.Radius >= RightBorder ||
                obj_1.Position.X - obj_1.Radius <= LeftBorder)
            {
                obj_1.Position.X -= (k_x + 0.5f) * obj_1.Velocity.X;
                obj_1.Velocity.X *= -1;
            }

            // Вычисление скорости по оси Y при соудорении с нижней или верхнёй стенкой И смещение шара назад на delta_y
            if (obj_1.Position.Y + obj_1.Radius >= DownBorder ||
                obj_1.Position.Y - obj_1.Radius <= UpBorder)
            {
                obj_1.Position.Y -= (k_y + 0.5f) * obj_1.Velocity.Y;
                obj_1.Velocity.Y *= -1;
            }
        }
        public override void CollisionHandler(Rect obj_1, Ball obj_2)
        {
